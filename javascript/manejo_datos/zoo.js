...
// contar animales del zoo
let count = zoo.species.reduce((acc, animaltype) => {
...
console.log(buscaEmpleado('b0dc644a-5335-489b-8a2c-4e086c7819a2'));
...
console.log(buscaEmpleados(['c1f50212-35a6-4ecd-8223-f835538526c2', '4b40a139-d4dc-4f09-822d-ec25e819a5ad']))

// sacar nombres
let nombres = []
zoo.species.forEach(animaltype => {
    const typenames = animaltype.residents.map(resident => resident.name)
    nombres = [...nombres, ...typenames] //concatena arrays
    // nombres = nombres.concat(typenames) otra manera de concatenar 'concat()'
    // nombres = nombres.push(...typenames) // otra opción que hace lo mismo
})
...
const ticketTotalCalculator = (info) => {
    return Object.entries(info).reduce((acc, [group, count]) => {
        return acc + count * zoo.prices[group.toLowerCase()]; 
    },0);
};
